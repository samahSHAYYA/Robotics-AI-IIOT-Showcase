...
Validates that completed work meets acceptance criteria through
...
3. **Validate** acceptance criteria one by one. Mark each as
...
- **Conditional pass**: minor non-functional issues
  documented, no criteria failed.
- **Fail**: one or more criteria failed, or a regression
  introduced.

On fail, return to Orchestrator with full report. Orchestrator
decides whether to send back to Developer or adjust criteria.
